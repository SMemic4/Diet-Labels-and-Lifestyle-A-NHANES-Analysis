...
```{r 5 by 5 table for eduction and labels}
...
### Creating the Table for Analysis
...
expcount$expected %>% gt() %>% tab_options(table.width = pct(100)) %>% cols_align(align = "center") %>% fmt_number(decimals = 2)
...
### The Association Plot and Table for the 5x5 Table
...
```{r observed minus expected frequency table}
...
tbs_1 %>% gt() %>% tab_options(table.width = pct(100)) %>% cols_align(align = "center") %>% fmt_number(decimals = 2) %>% cols_label(Education = "")

```


From the table, we observe that college graduates and those with some college education have a higher observed frequency of reading nutrition labels before purchasing a food product and are less likely to read the label only sometimes, rarely, or never. Conversely, individuals with a high school education or less are generally less likely to read nutrition labels frequently, except for those with less than a 9th-grade education, who show an unexpectedly higher frequency of reading labels always. These findings further support the association between education level and nutrition label reading behavior, indicating that higher education is linked to more frequent nutrition label usage.
